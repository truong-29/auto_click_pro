...
class UpdateDialog:
    ...
    def __init__(self, parent, update_info, on_update: callable, on_skip: callable):
        self.parent = parent
    ...
    def show(self):
        ...
        # Header với icon
        header = tk.Frame(self.dialog, bg=COLORS["accent_primary"], height=60)
        ...
        # Version info
        version_frame = tk.Frame(content, bg=COLORS["bg_secondary"])
        version_frame.pack(fill=tk.X, pady=(0, 10))
        
        tk.Label(
            version_frame,
            text=f"Phiên bản mới: {self.update_info.version}",
            font=FONTS["heading"],
            fg=COLORS["text_accent"],
            bg=COLORS["bg_secondary"],
            padx=15, pady=10
        ...
        notes_frame = tk.Frame(content, bg=COLORS["bg_input"])
        ...
        notes_text.insert("1.0", self.update_info.release_notes or "Không có ghi chú.")
        notes_text.config(state="disabled")
        
        # Progress bar (ẩn ban đầu)
        self.progress_frame = tk.Frame(content, bg=COLORS["bg_primary"])
        ...
        # Buttons
        self.btn_frame = tk.Frame(content, bg=COLORS["bg_primary"])
    ...
    def update_progress(self, percent: int):
        """Cập nhật tiến trình tải (gọi từ main thread)"""
        if self.progress_label and self.progress_fill:
            self.progress_label.config(text=f"Đang tải: {percent}%")
            self.progress_fill.place(x=0, y=0, relheight=1, relwidth=percent/100)
    
    def show_progress(self):
        """Hiển thị progress bar và disable buttons"""
